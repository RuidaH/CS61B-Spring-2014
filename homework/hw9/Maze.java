...
public class Maze {
  ...
  private static final int HWALL = 5;
  private static final int VWALL = 6;
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    /**
     * The rule of putting hWalls and vWalls in walls.
     * All hWalls first, then all vWalls.
     */
      
      int index, cur, counter = (vert - 1) * horiz + (horiz - 1) * vert;
      DisjointSets s = new DisjointSets(horiz * vert);
      int[] result, walls = new int[counter];
      int x, y, temp, edge;
      
      for (i = 0; i < counter; i++) walls[i] = i;
      
      while (counter != 1) {
          index = randInt(counter);  // From 0 to counter - 1
          cur = walls[counter - 1];
          walls[counter - 1] = walls[index];
          walls[index] = cur;
          counter--;
      }
      ...
      for (i = 0; i < walls.length; i++) {
          ...
          // Result stores the coordinate of cells that's connected by the edge
          result = new int[3];
          edge = walls[i];
          
          if (edge < temp) {        // edge is from hWalls
              x = (int)Math.floor(edge / horiz);
              y = edge % horiz;
              result[0] = x * horiz + y;
              result[1] = (x + 1) * horiz + y;
              result[2] = HWALL;
          } else {                 // from vWalls
              edge = edge - temp;
              x = (int)Math.floor(edge / (horiz - 1));
              y = edge % (horiz - 1);
              result[0] = x * horiz + y;
              result[1] = result[0] + 1;
              result[2] = VWALL;
          ...
          if (s.find(result[0]) != s.find(result[1])) { // Not in the same set
              s.union(result[0], result[1]);
              if (result[2] == HWALL) {
                  hWalls[y][x] = false;
              } else {
                  vWalls[y][x] = false;
  ...
  protected boolean depthFirstSearch(int x, int y, int fromWhere,
    ...
    // If you know it's from right, then you don't have to turn right and check that node.
