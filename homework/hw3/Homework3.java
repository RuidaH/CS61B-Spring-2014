...
//      int index = 0, idx = 0;
//
//      for (int i = 0; i < ints.length; i++) arr[i] = -1;
//
//      while (index < ints.length) {
//          arr[idx++] = ints[index++];
//          while(index < ints.length && arr[idx - 1] == ints[index]) {
//              index++;
//          }
//      }
//
//      for (int i = 0; i < ints.length; i++) ints[i] = arr[i];
//  }
  
  // Operate on the original array
  // public static void smoosh(int[] ints) {
      
  //     int nonDupCount = 0, idx = 0;
  //     while (idx < ints.length) {
  //         while (idx < ints.length && ints[nonDupCount] == ints[idx]) {
  //             idx++;
  //         }
  //         nonDupCount++;
  //         if (idx < ints.length) ints[nonDupCount] = ints[idx];
  //     }
      
  //     for (int i = nonDupCount; i < ints.length; i++) ints[i] = -1;
