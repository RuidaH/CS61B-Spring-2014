...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
      ...
      if (isValidNode()) {
          ...
          if (prev == null) {                     // Node has no children
              firstChild = newchild;
          } else if (prev == cur) {  // Inserting the node as the first child
              firstChild = newchild;
  ...
  public void removeLeaf() throws InvalidNodeException {
      ...
      if (isValidNode()) {
          if (firstChild == null) {
              this.myTree.size--;
              SibTreeNode cur = null, prev = null;
              
              if (this.parent == null) {  // If the node is the root
                  this.myTree.root = null;
              } else {
                  cur = parent.firstChild;

                  if (cur == this) {  // the leaf is the first child of its parent
                      cur.valid = false;
                      ...
                      while (cur.nextSibling != null) {
                          ...
                          if (cur == this) {
                              cur.valid = false;
                              if (cur.nextSibling == null) { // If the leaf is the last child of its parent
