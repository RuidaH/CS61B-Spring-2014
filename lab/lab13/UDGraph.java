...
public class UDGraph
{
  ...
  public UDGraph length2Paths() {
    ...
    public UDGraph length2Paths() {
        ...
        int[] degreeNumber = new int[vertices];
        UDGraph newGraph = new UDGraph(vertices);
        int x, y, z, i;
        
        // Get all the degree of each vertex
        ...
        int[][] incidentDegrees = new int[vertices][];
        
        // Get all the incidentEdge for each vertex
        for (x = 0; x < vertices; x++) {
            // incidentDegrees[i].length == degree of i (save memory)
            incidentDegrees[x] = new int[degreeNumber[x]];
            i = 0;
            for (y = 0; y < vertices; y++) {
                if (hasEdge(y, x)) {
                    incidentDegrees[x][i] = y;
                    i++;
                }
            }
        }
        ...
        for (x = 0; x < vertices; x++) {
            for (y = 0; y < incidentDegrees[x].length; y++) {
                int temp = incidentDegrees[x][y];
                for (z = 0; z < incidentDegrees[temp].length; z++) {
                    newGraph.addEdge(incidentDegrees[temp][z], x);
                }
            }
        }
   ...
   *  @return the new UDGraph.
   */
  // You cannot run length2Paths() every time
  // Because 1 --> 2 --> 4 --> 8
    /*
  public UDGraph paths(int length) {

      UDGraph newGraph = null, cur = null;

      for (int i = 1; i < length; i++) {
          if (i == 1) {
              cur = this.length2Paths();
          } else {
              newGraph = cur;
              cur = new UDGraph(vertices);
              for (int x = 0; x < vertices; x++) {
                  for (int y = 0; y < vertices; y++) {
                      if (newGraph.adjMatrix[x][y]) {
                          for (int z = 0; z < vertices; z++) {
                              if (this.adjMatrix[y][z]) {
                                  cur.addEdge(x, z);
                              }
                          }
                      }
    ...
    // because e.g. <u, v, w, u, v, w> is a valid length-5 path.
    // you cannot count this kind of path in this way.
    public UDGraph paths(int length) {
        // For every length from 1 to k, you have to create a new graph
        //      without any edges
        UDGraph newGraph = new UDGraph(vertices);
