...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
      ...
      if ((key.compareTo(node.entry.key()) < 0) && (node.leftChild != null)) {
          return findHelper(key, node.leftChild);
      } else if ((key.compareTo(node.entry.key()) > 0) && (node.rightChild != null)){
  ...
  public Entry remove(Object key) {
      ...
      if (node == null) {
          ...
          Entry temp = new Entry(node.entry.key(), node.entry.value());

          if (node.leftChild == null && node.rightChild == null) {  // When the node has no children
              if (node == root) {
                  root = null;
              } else if (((Comparable)node.entry.key()).compareTo(parent.entry.key()) > 0) {
                  parent.rightChild = null;
              } else {
                  parent.leftChild = null;
              }
          } else if (node.leftChild != null && node.rightChild == null) {  // When the node has only one children
              ...
              // Find the smallest key in the right-subtree of the node
