...
public class SplayTree implements Dictionary {
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    while (node.parent != null) {
        ...
        if (node.parent == root) {
            zig(node);
        } else if ((p.leftChild == node && p == p.parent.leftChild)
                   || (p.rightChild == node && p == p.parent.rightChild)) {
            zigZig(node);
        } else {
            zigZag(node);
        }
   ...
   **/
    public Entry remove(Object key) {
        BinaryTreeNode node = removeHelper((Comparable)key, root, null);
        if (node == null)
            return null;
        else
            return node.entry;
    ...
    private BinaryTreeNode removeHelper(Comparable key, BinaryTreeNode node, BinaryTreeNode theParent) {
        ...
        if (key.compareTo(node.entry.key()) < 0) {
            ...
            removeHelper(key, node.leftChild, node);
        } else if (key.compareTo(node.entry.key()) > 0) {
            if (node.rightChild == null) {
                splayNode(node);
                return null;
            }
            removeHelper(key, node.rightChild, node);
        } else {  // Find the key, splay key's parent
            if (node.leftChild == null && node.rightChild == null) {
                if (theParent.leftChild == node)
                    theParent.leftChild = null;
                else
                    theParent.rightChild = null;
            } else if (node.leftChild != null && node.rightChild == null) {
                if (theParent.leftChild == node)
                    theParent.leftChild = node.leftChild;
                else
                    theParent.rightChild = node.leftChild;
                node.leftChild.parent = theParent;
            } else if (node.leftChild == null && node.rightChild != null) {
                if (theParent.leftChild == node)
                    theParent.leftChild = node.rightChild;
                else
                    theParent.rightChild = node.rightChild;
                node.rightChild.parent = theParent;
            } else {
                BinaryTreeNode temp, cur = node.rightChild;
